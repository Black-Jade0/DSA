...
void solve()
{
    ...
    for (int i = 0; i < m; i++)
    {
        if (op[i].first <= track.first && op[i].second <= track.second && op[i].second >= track.first)
        {
            track.first = op[i].first;
        }
        else if (op[i].first >= track.first && op[i].second >= track.second && op[i].first <= track.second)
        {
            track.second = op[i].second;
        }
        else if (op[i].first <= track.first && op[i].second >= track.second)
        {
            track.first = op[i].first;
            track.second = op[i].second;
        }
    }
    cout << track.second - track.first + 1 << endl;
    return;
}
