...
void solve()
{
    ...
    {
        for (int j = 0; j < track[check[c[i]]].size(); j++)
        {
            cout << track[check[c[i]]][j] + 1 << " " << track[check[c[i + 1]]][0] + 1 << endl;
        }
        i++;
    }
    int i = c.size() - 1;
    for (int j = 1; j < track[check[c[i]]].size(); j++)
    {
        cout << track[check[c[i]]][j] + 1 << " " << track[check[c[0]]][0] + 1 << endl;
    }
    return;
}
